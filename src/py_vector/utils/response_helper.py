...
class ResponseHelper:
    ...
    @staticmethod
    def paginated(
        items: list, total: int, page: int, page_size: int, message: str = "获取成功"
    ) -> JSONResponse:
        """返回分页响应"""
    ...
    def bad_request(message: str = "请求参数错误") -> JSONResponse:
        """返回400响应"""
        return ResponseHelper.error(
            message=message, error_code="BAD_REQUEST", status_code=400
        )
    ...
    def created(data: Any = None, message: str = "创建成功") -> JSONResponse:
        """返回201响应"""
        return ResponseHelper.success(data=data, message=message, status_code=201)

    @staticmethod
    def accepted(data: Any = None, message: str = "请求已接受") -> JSONResponse:
        """返回202响应（异步处理）"""
        return ResponseHelper.success(data=data, message=message, status_code=202)
